...
Ui_button::Ui_button(UI_type type, p2Point<int> pos, bool inter, p2SString centered_text, j1Module * listener, int font_size, SDL_Color _color)
{
	...
	on_hover_tex = App->gui->botton_h; //gcambiar cuando pases a tu programa tendras que cambiar esto.
	normal = App->gui->botton_n;
	on_pressed_tex = App->gui->botton_p;
...
void Ui_button::CalculateCenter()
{
	...
	text_pos.x = my_pos.x + (normal.w / 2) - (text_width);
